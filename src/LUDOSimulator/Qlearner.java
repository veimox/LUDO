 ...
 *
 */
public class Qlearner implements LUDOPlayer {
	...
	int[] action;
	int[] new_action;
	public Qlearner(LUDOBoard board, int game)
	{
		this.game = game;
		this.board = board;
		rand = new Random();
	...
	public void play() {
		...
		if(rand.nextFloat()<10/(this.game+1) && max!=-1){
		...
		if(bestIndex!=-1 && state[bestIndex]>=0 && action[bestIndex]>=0){
			...
			double dQ=0.9*(reward(state[bestIndex], action[bestIndex], new_state[bestIndex])+0.2*board.Qtable[new_state[bestIndex]][new_action[bestIndex]]-board.Qtable[state[bestIndex]][action[bestIndex]]);
